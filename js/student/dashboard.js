...
async function initializeDashboard() {
    console.log('🎯 대시보드 초기화 시작 - 통합 버전 v1.6.0');
...
/**
 * 🆕 문화교구 신청 페이지로 이동 - 수업계획 상태 기반 분기 지원
 * 
 * 동작 방식:
 * 1. equipment-request.html로 이동
 * 2. equipment-request.html에서 수업계획 상태 확인
 * 3. 미제출자: lessonPlanPage 활성화 (create 모드)
 * 4. 제출자: studentPage 활성화 (교구신청 화면)
 */
function navigateToEquipmentRequest() {
    try {
        console.log('📋 문화교구 신청 페이지로 이동 - 수업계획 상태 기반 분기 지원');
        console.log('💡 equipment-request.html에서 수업계획 상태를 확인하여 적절한 화면을 표시합니다.');
        
        // 데이터 동기화
        ensureDataSync();
        
        // equipment-request.html로 이동 (여기서 수업계획 상태 확인 후 분기)
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🎯 대시보드 페이지 로드 완료 - 통합 버전 v1.6.0');
