...
// 🔧 삭제하고 재신청 버튼 기능 완성 - 실제 API 호출 구현
...
class FlightRequestUI {
    ...
    async init() {
        try {
            console.log('🔄 FlightRequestUI v8.7.3 초기화 시작 (삭제하고 재신청 버튼 기능 완성)...');
            ...
            console.log('✅ FlightRequestUI v8.7.3 초기화 완료 - 삭제하고 재신청 버튼 기능 완성');
        ...
        return new Promise((resolve, reject) => {
            const check = () => {
                ...
                if (apiExists && apiInitialized && utilsReady) {
                    ...
                    console.log('✅ [UI디버그] FlightRequestUI v8.7.3 의존성 로드 완료');
    ...
    async loadInitialData() {
        try {
            console.log('🔄 [UI디버그] v8.7.3 초기 데이터 로드 시작 - 삭제하고 재신청 버튼 기능 완성');
    ...
    showFlightRequestPageWithoutData() {
        console.log('🔄 [UI디버그] v8.7.3 기본 항공권 신청 페이지 표시 (데이터 없음)');
    ...
    async showPassportInfoPage() {
        try {
            console.log('🔄 [UI디버그] v8.7.3 여권정보 페이지 표시...');
            ...
            console.log('✅ [UI디버그] v8.7.3 여권정보 페이지 표시 완료');
        } catch (error) {
    ...
    async initializePassportInfoUI() {
        try {
            console.log('🔧 [UI디버그] v8.7.3 여권정보 UI 초기화 시작 (자동 폼 채우기 기능)');
            ...
            console.log('✅ [UI디버그] v8.7.3 여권정보 UI 초기화 완료');
        } catch (error) {
    ...
    async loadExistingPassportDataAndSetMode() {
        try {
            console.log('🔄 [UI디버그] v8.7.3 기존 여권정보 로드 및 모드 설정 시작...');
            
            const passportInfo = await window.flightRequestAPI.getPassportInfo();
            
            if (passportInfo) {
                console.log('✅ [UI디버그] v8.7.3 기존 여권정보 발견 - 폼 채우기 실행');
                await this.populatePassportForm(passportInfo);
                return true;
            } else {
                console.log('ℹ️ [UI디버그] v8.7.3 기존 여권정보 없음 - 신규 등록 모드');
                return false;
            }
        } catch (error) {
            console.error('❌ [UI디버그] v8.7.3 여권정보 로딩 실패:', error);
            return false;
        }
    }
    ...
    async populatePassportForm(passportData) {
        try {
            console.log('🔄 [UI디버그] v8.7.3 여권정보 폼 채우기 시작...', passportData);
            ...
            console.log('✅ [UI디버그] v8.7.3 여권정보 폼 채우기 완료');
            return true;
        } catch (error) {
            console.error('❌ [UI디버그] v8.7.3 폼 채우기 실패:', error);
            return false;
        }
    }
    ...
    closePassportView() {
        console.log('🔄 [UI디버그] v8.7.3 여권정보 보기 닫기 - 항공권 신청 페이지로 이동');
        this.showFlightRequestPage();
    ...
    editPassportInfo() {
        console.log('🔄 [UI디버그] v8.7.3 여권정보 편집 모드로 전환');
    ...
    async handlePassportSubmit(event) {
        ...
        try {
            console.log('🔄 [UI디버그] v8.7.3 여권정보 제출 시작...');
    ...
    async loadFlightRequestData() {
        ...
        try {
            ...
            console.log('🔄 [UI디버그] 항공권 신청 데이터 로드 시작 (v8.7.3 - 삭제하고 재신청 버튼 기능 완성)');
            ...
            if (this.existingRequest) {
                ...
                console.log('✅ [UI디버그] v8.7.3: 기존 신청 발견 - 상태별 정보 표시:', this.existingRequest.status);
                this.showExistingRequest();
            } else {
                // 새 신청 폼 표시
                console.log('✅ [UI디버그] v8.7.3: 신규 신청 - 신청 폼 표시');
                this.showRequestForm(false);
            }

            console.log('✅ [UI디버그] 항공권 신청 데이터 로드 완료 (v8.7.3)');
        } catch (error) {
    ...
    handlePurchaseTypeChange() {
        ...
        console.log('🔧 [UI디버그] v8.7.3: 구매 방식 변경:', selectedType);
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            console.log('🔍 [UI디버그] v8.7.3 제출 데이터 (가격 정보 포함):', {
                ...requestData,
    ...
    async deleteRequest(requestId) {
        try {
            console.log('🗑️ [UI디버그] v8.7.3 삭제 요청 시작:', requestId);
            ...
            if (!confirm(confirmMessage)) {
                console.log('🔄 [UI디버그] v8.7.3 사용자가 삭제 취소');
                return;
            }

            console.log('🗑️ [UI디버그] v8.7.3 실제 삭제 API 호출 시작...');
            ...
            // 🔧 v8.7.3: 실제 API 삭제 메서드 호출
            const result = await this.api.deleteFlightRequest(requestId);

            console.log('✅ [UI디버그] v8.7.3 삭제 API 성공:', result);
            ...
            setTimeout(() => {
                console.log('🔄 [UI디버그] v8.7.3 삭제 완료 후 페이지 새로고침');
                window.location.reload();
            }, 2000);

        } catch (error) {
            console.error('❌ [UI디버그] v8.7.3 삭제 실패:', error);
...
// 🔧 v8.7.3: FlightRequestUI 클래스를 전역 스코프에 노출
window.FlightRequestUI = FlightRequestUI;

// 페이지 로드 시 초기화 제거 - HTML에서 모듈 로딩 완료 후 초기화
console.log('✅ FlightRequestUI v8.7.3 모듈 로드 완료 - 삭제하고 재신청 버튼 기능 완성');
