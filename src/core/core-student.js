/**
 * 학생 핵심 시스템 - Claude 최적화 구조
 * 
 * @description 학생 대시보드, 교구 신청, 예산 관리 및 수업계획 연동
 * @dependencies AuthManager (src/core/core-auth.js), SupabaseAPI (src/core/core-supabase.js), App (src/core/core-app.js)
 * @author Claude AI
 * @date 2024-12-16
 */
        ...
        try {
            ...
            try {
                if (this.currentEditingItem) {
                    ...
                    // 새 신청 모드
                    await SupabaseAPI.addApplication(currentUser.id, formData);
    ...
    getApplicationFormData() {
        try {
            const formData = {
                ...
                purchaseMethod: document.querySelector('input[name="purchaseMethod"]:checked')?.value || 'online',
                type: 'single'
            };
    ...
    getBundleFormData() {
        try {
            const formData = {
                ...
                link: document.getElementById('bundleLink')?.value?.trim() || '',
                purchaseMethod: 'online', // 묶음은 항상 온라인
                type: 'bundle',
                bundleInfo: {
            ...
            if (!formData.bundleInfo.user_id) {
                alert('계정 ID를 입력해주세요.');
                document.getElementById('bundleUserId')?.focus();
                return null;
            }

            if (!formData.bundleInfo.password) {
                alert('비밀번호를 입력해주세요.');
...
console.log('📚 Student core system loaded successfully - Claude Optimized Structure');
